...
/**
 * Admin Order Detail — /admin/orders/:id  (MongoDB _id)
 *
 * Fetches a single order from GET /api/orders/id/:id.
 * Allows admins to update status, payment status, and add timeline notes.
 */
...
export default function AdminOrderDetailPage() {
    // The [...slug] resolver passes the full path; last segment is the _id
        ...
        <div className="space-y-6 max-w-6xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* Payment proof */}
                    {order.paymentProof && (order.paymentProof.transactionId || order.paymentProof.paymentInfo || order.paymentProof.proofImage) && (
                        <Card title="Payment Proof" icon="mdi:credit-card-check-outline">
                            <div className="space-y-2 text-sm">
                                {order.paymentProof.transactionId && (
                                    <div className="flex gap-2">
                                        <span className="text-gray-500 w-32 shrink-0">Transaction ID</span>
                                        <span className="font-mono text-gray-800 break-all">{order.paymentProof.transactionId}</span>
                                    </div>
                                )}
                                {order.paymentProof.paymentInfo && (
                                    <div className="flex gap-2">
                                        <span className="text-gray-500 w-32 shrink-0">Payment Info</span>
                                        <span className="text-gray-800">{order.paymentProof.paymentInfo}</span>
                                    </div>
                                )}
                                {order.paymentProof.proofImage && (
                                    <div>
                                        <p className="text-gray-500 mb-2">Screenshot</p>
                                        <div className="relative w-full max-w-xs aspect-video rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                ...
                <div className="space-y-6">
                    ...
                    <Card title="Summary" icon="mdi:receipt-text-outline">
                        <dl className="space-y-2 text-sm">
                            ...
                            <Row label="Customer"  value={order.userEmail || order.shippingAddress.email || "Guest"} />
                            <Row label="Payment"   value={<span className="capitalize">{(order.paymentGatewayType ?? "—").replace(/_/g, " ")}</span>} />
                            <Row label="Shipping"  value={<span className="capitalize">{order.shippingMethod}</span>} />
                            <Row label="Inventory" value={order.inventoryUpdated ? "Updated" : "Pending"} />
                        </dl>
                    </Card>
